TileEntity.registerPrototype(BlockID.alvearyController, {
        ...
        if (World.getThreadTime() % 40 === 0
            ...
            if (!this.data.valid) {
                for (const dir of directions) {
                    this.blockSource.setBlock(this.x + dir.x, this.y + dir.y, this.z + dir.z, BlockID.alvearyMisc, 0);
                }
            }
            ...
            for (let xx = this.x - 1; xx < this.x + 3; xx++) {
                for (let yy = this.y - 2; yy < this.y + 3; yy++) {
                    for (let zz = this.z - 1; zz < this.z + 3; zz++) {
                        ...
                        if (ApiaryRegistry.isApiaryComponent(block) && block !== BlockID.alvearyController) {
