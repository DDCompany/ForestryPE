...
function generateOre(blockId: number, chunkX: number, chunkZ: number, inChunk: number, size: number, minY: number, maxY: number, biomes?: number[]) {
    for (let i = 0; i < inChunk; i++) {
        ...
        if (!biomes || biomes.indexOf(World.getBiome(coords.x, coords.z)) > -1) {
            GenerationUtils.generateOre(coords.x, coords.y, coords.z, blockId, 0, Util.random(1, size), false);
        }
    }
}
