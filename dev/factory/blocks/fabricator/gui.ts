...
{
    const elements = fabricatorGUI.getContent().elements!!;

    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            elements[`slotInput${i * 3 + j}`] = {type: "slot", x: 541 + j * 60, y: 88 + i * 60};
        }
    }

    for (let i = 0; i < 2; i++) {
        for (let j = 0; j < 9; j++) {
            elements[`slotResources${i * 9 + j}`] = {type: "slot", x: 335 + j * 60, y: 281 + i * 60};
        }
    }
}