IDRegistry.genBlockID("fermenter");
FactoryManager.register("fermenter", "forestry.block.fermenter", [
    ["fermenter_top", 0], ["fermenter_top", 0], ["fermenter", 0], ["fermenter", 0], ["fermenter_side", 0], ["fermenter_side", 0]
], true);
GROUP_ITEM_PIPE.add(BlockID.fermenter, -1);

Callback.addCallback("PostLoaded", () => {
    Recipes.addShaped({id: BlockID.fermenter, count: 1, data: 0}, [
        "cgc",
        "gmg",
        "cgc"
    ], ['c', ItemID.gearBronze, -1, 'g', VanillaBlockID.glass, -1, 'm', ItemID.sturdyMachine, -1]);
});