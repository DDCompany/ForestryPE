...
{
    const content = carpenterGUI.getContent().elements!!;

    for (let i = 0; i < 3; i++) {
        for (let j = 0; j < 3; j++) {
            content[`slotInput${i * 3 + j}`] = {type: "slot", x: 385 + j * 60, y: 75 + i * 60};
        }
    }

    for (let i = 0; i < 2; i++) {
        for (let j = 0; j < 9; j++) {
            content[`slotResources${i * 9 + j}`] = {type: "slot", x: 335 + j * 60, y: 281 + i * 60};
        }
    }
}
